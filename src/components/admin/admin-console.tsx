...
type ModuleKey = "settings" | "customers" | "categories" | "filters" | "products" | "admins";
...
export function AdminConsole({ admin, initialData }: { admin: AdminProfile; initialData: AdminDashboardData }) {
  ...
  const submitSettings = async (values: StoreData["settings"]) => {
    ...
    const payload = await response.json();
    if (!response.ok) {
      message.error(payload.message || "保存设置失败");
      return;
    }
    ...
    { title: "状态", dataIndex: "isActive", render: (value) => <Tag color={value ? "green" : "default"}>{value ? "启用" : "停用"}</Tag> },
    {
      ...
      render: (_, record) => (
        <Space wrap>
          ...
          <Popconfirm title="确认删除该筛选组？" onConfirm={() => removeFilter(record.id)}>
            <Button danger type="link">删除</Button>
          </Popconfirm>
        </Space>
      ),
  ...
  const moduleDescriptions: Record<ModuleKey, string> = {
    settings: "配置店铺基础信息、首页文案与下单收款信息。",
    customers: `已注册用户数：${data.customers.length}`,
    products: `商品总数：${data.store.products.length}`,
    categories: `分类总数：${data.store.categories.length}`,
    filters: `筛选组总数：${data.store.filterGroups.length}`,
    admins: `管理员账号数：${data.admins.length}`,
  };
      ...
      >
        <div style={{ display: "flex", minHeight: "100dvh", flexDirection: "column", gap: 16 }}>
          <div style={{ borderBottom: "1px solid #f0f0f0", padding: "4px 12px 12px" }}>
            ...
            <Typography.Text type="secondary">NextMail 管控台</Typography.Text>
          </div>
      ...
      <Layout.Content style={{ minWidth: 0, padding: "16px 0 24px" }}>
            ...
            <CardSection title={moduleTitles.settings}>
              <Form form={settingsForm} initialValues={data.store.settings} layout="vertical" onFinish={submitSettings}>
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <Form.Item label="订单链接" name="orderLink"><Input /></Form.Item>
                  <Form.Item label="客服邮箱" name="supportEmail"><Input /></Form.Item>
                  <Form.Item label="客服电话" name="supportPhone"><Input /></Form.Item>
                  <Form.Item label="收款户名" name="paymentAccountName"><Input /></Form.Item>
                  <Form.Item label="收款账号" name="paymentAccountNumber"><Input /></Form.Item>
                  <Form.Item label="收款银行" name="paymentBankName"><Input /></Form.Item>
                </div>

                <Form.Item label="首页主标题" name="heroTitle"><Input /></Form.Item>
                <Form.Item label="首页副标题" name="heroSubtitle"><Input.TextArea rows={2} /></Form.Item>
                <Form.Item label="首页公告" name="heroNotice"><Input.TextArea rows={2} /></Form.Item>
                <Form.Item label="购买说明" name="purchaseGuide"><Input.TextArea rows={4} /></Form.Item>
                  ...
                  <Space orientation="vertical" size={16} style={{ display: "flex" }}>
                    {fields.map((field, index) => (
                      <Card key={field.key} size="small" title={`选项 ${index + 1}`} extra={<Button danger type="link" onClick={() => remove(field.name)}>删除</Button>}>
                        ...
                        <Form.Item label="名称" name={[field.name, "label"]} rules={[{ required: true }]}><Input /></Form.Item>
        ...
        <Modal centered destroyOnHidden footer={null} onCancel={() => setProductModalOpen(false)} open={productModalOpen} title={editingProductId ? "编辑商品" : "新增商品"} width={860}>
          <Form form={productForm} layout="vertical" onFinish={submitProduct}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <Form.Item label="角标" name="badge"><Input /></Form.Item>
              ...
              >
                <Space direction="vertical" size={12} style={{ display: "flex", width: "100%" }}>
                  ...
                  <Input placeholder="上传后图片地址会显示在这里" readOnly value={productImageUrl} />
                  {productImageUrl ? (
                    <div
                      style={{
            ...
            <Form.Item label="筛选绑定" name="filterOptionIds"><Select mode="multiple" options={flatFilterOptions} placeholder="选择适用于该商品的筛选条件" /></Form.Item>
        ...
        <Modal centered destroyOnHidden footer={null} onCancel={() => setAdminModalOpen(false)} open={adminModalOpen} title="管理员">
          <Form form={adminForm} layout="vertical" onFinish={submitAdmin}>
            ...
            <Form.Item label="密码（编辑时留空则不修改）" name="password"><Input.Password /></Form.Item>
            <Form.Item initialValue={true} label="启用" name="isActive" valuePropName="checked"><Switch /></Form.Item>
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 24 }}>
